...
function code()
	if emu.read(ram_World_Num, emu.memType.cpu) == 0 then
		if emu.getState().ppu.frameCount < 39 then --Less than 39 frames in, hammer brothers moving is impossible
			drawString(1, 9, "Facing?: N/A", text_colour, text_back_colour)
			drawString(1, 18, "Moving?: N/A", text_colour, text_back_colour)
		else
			if emu.read(ram_Map_Objects_IDs + 2, emu.memType.cpu) == 3 or emu.read(ram_Map_Objects_IDs + 2, emu.memType.cpu) == 0xB
			or (emu.read(ram_Map_Objects_XLo + 2, emu.memType.cpu) == 0 and emu.read(ram_Map_Objects_Y + 2, emu.memType.cpu) == 0) then
				if emu.read(ram_Map_Operation, emu.memType.cpu) == 0xB and PreviousMap_Operation ~= 0xB then
					colour = text_move_colour
				else
					colour = text_colour
				end
			end
			PreviousMap_Operation = emu.read(ram_Map_Operation, emu.memType.cpu)
			
			if emu.read(ram_Level_Tileset, emu.memType.cpu) ~= 0 or emu.read(ram_Map_Operation, emu.memType.cpu) < 0xB then --If not in world map or before map hammer brother march operation
				if emu.read(ram_Map_Entered_Y, emu.memType.cpu) == 0x60 and emu.read(ram_Map_Entered_X, emu.memType.cpu) == 0x60 then --If player is in 1-F
					HB_X = 0xFF
					HB_Y = 0xFF
				else
					if emu.read(ram_Map_Objects_XLo + 2, emu.memType.cpu) == 0xA0 and emu.read(ram_Map_Objects_Y + 2, emu.memType.cpu) == 0x80 then
						HB_X = 0xA0
						HB_Y = 0x80
					elseif emu.read(ram_Map_Objects_XLo + 2, emu.memType.cpu) == 0x80 and emu.read(ram_Map_Objects_Y + 2, emu.memType.cpu) == 0x80 then
						HB_X = 0x80
						HB_Y = 0x80
					elseif emu.read(ram_Map_Objects_XLo + 2, emu.memType.cpu) == 0x60 and emu.read(ram_Map_Objects_Y + 2, emu.memType.cpu) == 0xA0 then
						HB_X = 0x60
						HB_Y = 0xA0
					elseif emu.read(ram_Map_Objects_XLo + 2, emu.memType.cpu) == 0x40 and emu.read(ram_Map_Objects_Y + 2, emu.memType.cpu) == 0x80 then
						HB_X = 0x40
						HB_Y = 0x80
					elseif emu.read(ram_Map_Objects_XLo + 2, emu.memType.cpu) == 0x40 and emu.read(ram_Map_Objects_Y + 2, emu.memType.cpu) == 0x60 then
						HB_X = 0x40
						HB_Y = 0x60
					end
				end
			...
			if emu.read(ram_Map_Objects_XLo + 2, emu.memType.cpu) == 0 and emu.read(ram_Map_Objects_Y + 2, emu.memType.cpu) == 0 then
				HB_X = 0xA0
				HB_Y = 0x80
			end
			...
			if HB_Y == 0xFF and HB_X == 0xFF then                                --If player is in 1-F,
				...
				Facing = (emu.read(ram_Random_Pool + 8, emu.memType.cpu) & 6) >> 1 --Facing direction is determined 39 frames before moving after completing numbered stages
				if Facing == 0 then
				...
				Value = emu.read(ram_Random_Pool + 3, emu.memType.cpu) & 0x83
				...
				elseif HB_X == 0x60 and HB_Y == 0xA0 then --If HB is between 1-5 and 1-6
					if emu.read(ram_Map_Entered_X, emu.memType.cpu) == 0x40 and emu.read(ram_Map_Entered_Y, emu.memType.cpu) == 0xA0 then --If player is in 1-5
						drawString(1, 18, "Moving?: RIGHT", text_back_colour, text_back_colour)
						emu.drawString(44, 18, "RIGHT", colour, 0xFF000000)
					else
						if (Facing == 0 and (Value == 0 or Value == 2 or Value == 3 or Value == 0x80))
				...
				elseif HB_X == 0x80 and HB_Y == 0x80 then --If HB is above 1-6
					if emu.read(ram_Map_Entered_X, emu.memType.cpu) == 0x80 and emu.read(ram_Map_Entered_Y, emu.memType.cpu) == 0xA0 then --If player is in 1-6
						if (Facing == 0 and (Value == 0 or Value == 2 or Value == 3 or Value == 0x80))
						or (Facing ~= 1 and (Value == 1 or Value == 0x81 or Value == 0x82 or Value == 0x83)) then
							drawString(1, 18, "Moving?: RIGHT", text_back_colour, text_back_colour)
							emu.drawString(44, 18, "RIGHT", colour, 0xFF000000)
